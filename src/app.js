...
export default function App() {
  ...
  // Adjust variables based on the device
  let touchRad = isMobile ? 10 : 60; 
  let rainRad = isMobile ? 4.5 : 25; 

  // Load background image with a condition for mobile
  const image = new Image();
  // On mobile, load a lower resolution image for performance optimization
  image.src = isMobile ? "/bg_resized Medium2.jpeg" : "/bg.png";
  ...
  function handleMouseMove() {
    ...
    for (let i = 0; i < density; i++) {
      drawingCanvas.graphics
        .setStrokeStyle(touchRad, "round", "round") // Set line thickness and end caps
  ...
  function createRaindrop(x, y) {
    let rad = rainRad; // Initial radius of the raindrop
    let startY = isMobile ? 2 : 20;
    let endY = isMobile ? 5 : 30;
    ...
    let dropDis = isMobile
      ? (canvas.height / 2) * (0.05 + Math.random() * 0.1) + y
      : canvas.height * (0.1 + Math.random() * 0.5) + y;

    // Create a tween animation for the raindrop's movement and scaling
    Tween.get(raindrop)
      .to(
        {
          y: y + (startY + Math.random() * 50),
      ...
      .to({ y: dropDis + endY, radius: rad * 0.55 }, 1000, Ease.linear)
      .call(() => {
        // Remove the raindrop from the stage to free up resources and prevent further rendering
        stage.removeChild(raindrop);

        // Remove the completed raindrop from the `raindrops` array
        // `filter()` creates a new array without the specified `raindrop`
        raindrops = raindrops.filter((drop) => drop !== raindrop);

        // If there are no remaining raindrops in the array, stop the Ticker to optimize performance
        if (raindrops.length === 0) {
          createjs.Ticker.removeEventListener("tick", updateRaindropMask);
        }
      });
